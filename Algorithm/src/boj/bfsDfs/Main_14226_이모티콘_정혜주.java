...
public class Main_14226_이모티콘_정혜주 {
	...
	public static void bfs(int s) {
		...
		while(!q.isEmpty()) {
			...
			//1. 클립보드에 복사
			q.offer(new Emozi(temp.screen, temp.screen, temp.time+1));
			...
			if(temp.clipboard!=0 &&  temp.screen + temp.clipboard <= S 
				...
				q.offer(new Emozi(temp.clipboard, temp.screen + temp.clipboard, temp.time+1));
			...
			if(temp.screen-1>=0 && !isVisited[temp.clipboard][temp.screen-1] ) {
				q.offer(new Emozi(temp.clipboard, temp.screen-1, temp.time+1));
